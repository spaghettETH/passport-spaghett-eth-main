  ...
  try {
    ...
    if (configs.network !== undefined && configs.owner_mnemonic !== undefined) {
      ...
      if (
        ...
        const { addresses, keys } = await derive(configs.owner_mnemonic, 4);
        configs.owner_key = keys[0];
        configs.owner_address = addresses[0];
        configs.minter_key = keys[1];
        configs.minter_address = addresses[1];
        configs.signer_key = keys[2];
        configs.signer_address = addresses[2];
        configs.setter_key = keys[3];
        configs.setter_address = addresses[3];
